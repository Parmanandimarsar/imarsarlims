...
import { MasterPatientDocumentColumns } from "../../../TableField/TablefieldsColumns";
...
const PatientDocumentMaster = () => {
  ...
  const validationSchema = Yup.object().shape({
    patientdocumentmaster: Yup.string().required("Patient Document is required"),
  ...
  const onSubmit = (values, { setSubmitting, resetForm }) => {
    if (editRow) {
      ...
      setRows([
        ...
        {
          id: rows.length + 1,
          patientdocumentmaster: values.patientdocumentmaster,
          ...values,
        },
      ]);
    }
  ...
  const exportToExcel = () => {
    const headers = ["S.No", "Patient Document Master",  "Active"];
    const data = rows.map((row) => [
      row.id,
      row.patientdocumentmaster,
      row.active ? "Active" : "Inactive",
    ]);

    ExportToExcel(data, headers, "Patient_Document_Master.xlsx");
  };
            ...
            <Form className="p-1">
                  ...
                  <FormControl fullWidth>
                    <Grid container alignItems="center">
                      <Grid
                        ...
                        sx={{ mr: "3px" }}
                      >
                        <FormLabel>Patient Document Master</FormLabel>
                      </Grid>
                      <Grid item xs={6}>
                        <FormControl fullWidth>
                          <Field
                            ...
                            name="patientdocumentmaster"
                            fullWidth
                            placeholder="Enter Discount Reason Master"
                            size="small"
                            error={
                              touched.patientdocumentmaster && !!errors.patientdocumentmaster
                            }
                          ></Field>
                          <ErrorMessage
                            name="patientdocumentmaster"
              ...
              <div className="h-[150px] w-full">
                <DataGrid
                  ...
                  rows={handleFilter()}
                  columns={MasterPatientDocumentColumns(
                    handleToggleActive,
                    handleEdit,
                    Switch
                  )}
