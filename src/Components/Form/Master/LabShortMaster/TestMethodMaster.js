...
import { MasterTestMethodMaster } from "../../../TableField/TablefieldsColumns";
...
const TestMethodMaster = () => {
  ...
  const validationSchema = Yup.object().shape({
    TestMethodMaster: Yup.string().required(" Out SourceLab Master is required"),
  });
  ...
  const onSubmit = (values, { setSubmitting, resetForm }) => {
    if (editRow) {
      ...
      setRows([
        ...
        {
          id: rows.length + 1,
          TestMethodMaster: values.TestMethodMaster,
          ...values,
        },
      ]);
    }
  ...
  const exportToExcel = () => {
    const headers = ["S.No", "Test Method Master", "Active"];
    const data = rows.map((row) => [
      row.id,
      row.TestMethodMaster,
      row.active ? "Active" : "Inactive",
    ]);

    ExportToExcel(data, headers, "Test_Method_Master.xlsx");
  };
            ...
            <Form className="p-1">
                  ...
                  <FormControl fullWidth>
                    <Grid container alignItems="center">
                      <Grid
                        ...
                        sx={{ mr: "3px" }}
                      >
                        <FormLabel>Test Method Master</FormLabel>
                      </Grid>
                      <Grid item xs={6}>
                        <FormControl fullWidth>
                          <Field
                            ...
                            fullWidth
                            placeholder="Enter Test Method Master"
                            size="small"
                            error={
                              touched.TestMethodMaster && !!errors.TestMethodMaster
                            }
                          ></Field>
                          <ErrorMessage
                            name="TestMethodMaster"
              ...
              <div className="h-[150px] w-full">
                <DataGrid
                  ...
                  rows={handleFilter()}
                  columns={MasterTestMethodMaster(
                    handleToggleActive,
                    handleEdit,
                    Switch
                  )}
...
export default TestMethodMaster;
