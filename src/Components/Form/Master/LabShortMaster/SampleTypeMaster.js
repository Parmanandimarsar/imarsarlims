...
import { MasterSampleTypeMaster } from "../../../TableField/TablefieldsColumns";
...
const SampleTypeMaster = () => {
  ...
  const validationSchema = Yup.object().shape({
    SampleTypeMaster: Yup.string().required(" Sample Type Master is required"),
  });
  ...
  const onSubmit = (values, { setSubmitting, resetForm }) => {
    if (editRow) {
      ...
      setRows([
        ...
        {
          id: rows.length + 1,
          SampleTypeMaster: values.SampleTypeMaster,
          ...values,
        },
      ]);
    }
  ...
  const exportToExcel = () => {
    ...
    const data = rows.map((row) => [
      row.id,
      row.SampleTypeMaster,
      row.active ? "Active" : "Inactive",
    ]);

    ExportToExcel(data, headers, "Sample_Type_Master.xlsx");
  };
            ...
            <Form className="p-1">
                  ...
                  <FormControl fullWidth>
                    <Grid container alignItems="center">
                      <Grid
                        ...
                        sx={{ mr: "3px" }}
                      >
                        <FormLabel>Sample Type Master</FormLabel>
                      </Grid>
                      <Grid item xs={6}>
                        <FormControl fullWidth>
                          <Field
                            ...
                            name="SampleTypeMaster"
                            fullWidth
                            placeholder="Enter Sample Type Master "
                            size="small"
                            error={
                              touched.SampleTypeMaster && !!errors.SampleTypeMaster
                            }
                          ></Field>
                          <ErrorMessage
                            name="SampleTypeMaster"
              ...
              <div className="h-[150px] w-full">
                <DataGrid
                  ...
                  rows={handleFilter()}
                  columns={MasterSampleTypeMaster(
                    handleToggleActive,
                    handleEdit,
                    Switch
                  )}
...
export default SampleTypeMaster;
